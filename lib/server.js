...
var loadComponentAndConfig = function(quiverComponents, configPath, callback) {
...
var runServer = function(quiverComponents, commandArgs, basePath, callback) {
  var configPath = commandArgs.config
  if(configPath) configPath = pathLib.join(basePath, configPath)

  loadComponentAndConfig(quiverComponents, configPath, function(err, config) {
    if(err) return callback(err)
...
var runModuleAsServer = function(quiverModule) {
  moduleLib.loadComponentsFromQuiverModule(quiverModule, function(err, quiverComponents) {
    ...
    runServer(quiverComponents, optimist.argv, process.cwd(), function(err, listenPort) {
      if(err) throw err
...
var runQuiverServer = function(commandArgs, basePath, callback) {
  ...
  moduleLib.loadQuiverComponentsFromPath(modulePath, function(err, quiverComponents) {
    if(err) return callback(err)

    runServer(quiverComponents, commandArgs, basePath, callback)
  })
}
